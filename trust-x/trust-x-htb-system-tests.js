...
function getBidRequestRegex() {
    return {
        method: 'POST',
        urlRegex: /grid\.bidswitch\.net\/hbjson\?sp=trustx/
    };
}
...
function validateBidRequest(request) {
    ...
    slotKeys.forEach(function (value, idx) {
        ...
        expect(imp.tagid).toEqual(slot.adSlotId);
    });
...
function validateTargeting(targetingMap) {
    expect(targetingMap).toEqual(jasmine.objectContaining({
        ix_trstx_cpm: jasmine.arrayContaining([jasmine.stringMatching(/300x250_\d+/), jasmine.stringMatching(/120x600_\d+/)]),
